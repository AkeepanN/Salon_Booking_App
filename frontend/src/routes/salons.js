...
async function attachSalonSummaries(salons) {
  ...
  const ownerIds = [
    ...new Set(
      salons
        .map((salon) => salon.owner_id)
        .filter((ownerId) => ownerId && ownerId !== 'undefined' && ownerId !== 'null')
...
router.get('/', async (req, res, next) => {
  try {
    ...
    const salons = await Salon.find(query)
      .select('owner_id name address phone board_photo_url imagePosition latitude longitude workingHours active status approval_status createdAt')
      .sort({ name: 1 })
      .limit(50);
...
router.patch('/:salonId/board-photo', auth, uploadSalonPhoto.single('board_photo'), async (req, res, next) => {
  try {
    ...
    if (!req.file && req.body.imagePosition === undefined) {
      return res.status(400).json({ message: 'Please choose an image or adjust the image position' });
    }

    if (req.body.imagePosition !== undefined) {
      const imagePosition = normalizeImagePosition(req.body.imagePosition);
      if (!imagePosition) {
        if (req.file) {
          await fs.promises.unlink(req.file.path).catch(() => {});
        }
        return res.status(400).json({ message: 'Invalid image position values' });
      }
      salon.imagePosition = imagePosition;
    }
