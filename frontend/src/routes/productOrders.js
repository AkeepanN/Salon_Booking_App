...
router.get('/provider', auth, requireRole('barber'), async (req, res, next) => {
  try {
    const orders = await populateOrders(
      ProductOrder.find({ barber_id: req.user._id }).sort({ createdAt: -1 })
    );
...
router.get('/salon/:salonId', auth, requireRole('barber'), async (req, res, next) => {
  try {
    const salon = await Salon.findOne({
    ...
    const orders = await populateOrders(
      ProductOrder.find({ salon_id: salon._id }).sort({ createdAt: -1 })
    );
