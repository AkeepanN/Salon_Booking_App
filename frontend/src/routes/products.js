...
function normalizeProduct(product) {
  ...
  return {
    ...data,
    imagePosition: {
      x: Number(data?.imagePosition?.x ?? 50),
      y: Number(data?.imagePosition?.y ?? 50),
      zoom: Number(data?.imagePosition?.zoom ?? 1),
    },
  };
}

function parseImagePosition(input) {
  let source = input;
...
router.post('/', auth, requireRole('barber'), uploadProductImage.single('image'), async (req, res, next) => {
  try {
    ...
    const parsedImagePosition = imagePosition !== undefined ? parseImagePosition(imagePosition) : undefined;
    ...
    if (imagePosition !== undefined && !parsedImagePosition) {
      if (req.file) {
        await fs.promises.unlink(req.file.path).catch(() => {});
      }
...
router.patch('/:id', auth, requireRole('barber'), uploadProductImage.single('image'), async (req, res, next) => {
  try {
    ...
    if (req.body.imagePosition !== undefined) {
      const imagePosition = parseImagePosition(req.body.imagePosition);
      if (!imagePosition) {
        if (req.file) {
          await fs.promises.unlink(req.file.path).catch(() => {});
        }
